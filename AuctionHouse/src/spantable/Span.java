...
/**
 * A class that defines a rectangular span of rows and/or columns that should be
 * treated as a single cell. The 'row' and 'column' fields refer to the top-left
 * corner of the spanned cell. The 'activeRow' and 'activeColumn' fields
 * determine the address of the content displayed in the span, and must be
 * located within the span. The 'height' and 'width' fields determine the height
 * and width of the spanned area.
 * 
...
public class Span {
	 ...
	 * 
	 * @param row
	 *            The top-most row of the span
	 * @param column
	 *            The left-most row of the span
	 * @param activeRow
	 *            The active row
	 * @param activeColumn
	 *            The active column
	...
	public Span(int row, int column, int activeRow, int activeColumn, int height, int width) {
		...
		if (!isDefined(activeRow, activeColumn)) {
			throw new IllegalArgumentException("The active row and column must be inside the span.");
		}
	}
	 ...
	 * @param other
	 *            The other span
	 * @return {@code true} if there is an overlap between this Span and the
	 *         other.
	 */
	public boolean intersects(Span other) {
		return isDefined(other.row, other.column) // Other top-left cell
				|| isDefined(other.row + other.height - 1, other.column + other.width - 1) // Other
																							// bottom-right
																							// cell
				|| other.isDefined(row, column) // My top-left cell
				|| other.isDefined(row + height - 1, column + width - 1); // My
																			// bottom-right
																			// cell
	}
	...
	public boolean isDefined(int row, int column) {
		return row >= this.row && row < (this.row + this.height) && column >= this.column
	...
	public boolean equals(Object o) {
		...
		if (o instanceof Span) {
			Span s = (Span) o;
			return row == s.row && column == s.column && height == s.height && width == s.width
					&& activeRow == s.activeRow && activeColumn == s.activeColumn;
		}
		return false;
	}
	...
	public int hashCode() {
		...
		if (_hc == 0) {
			...
			_hc = 65537 * row + 16417 * activeRow + 4111 * column + 1031 * activeColumn + 257
					* height + width;
