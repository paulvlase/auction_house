...
public class SpanTable extends JTable implements SpanListener {
	...
	/**
	 * Overrides the superclass getCellRect() to return the rectangle for the
	 * spanned cell if the row and column fall into the spanned cell's
	 * boundaries.
	 * 
	 * @param row
	 *            The row
	...
	public void spanAdded(SpanEvent sme) {
		...
		tableChanged(new TableModelEvent(getModel(), span.getRow(), span.getRow()
				+ span.getHeight() - 1));
	}
