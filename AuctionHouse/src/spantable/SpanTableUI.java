...
public class SpanTableUI extends BasicTableUI {
	...
	private void paintRow(int row, Graphics g) {
		...
		for (int col = 0; col < table.getColumnCount(); col++) {
			...
			if (cellRect.intersects(clipRect)) {
				// If a span is defined, only paint the active (top-left) cell.
				// Otherwise paint the cell.
				Span span = ((SpanTableModel) table.getModel()).getSpanModel().getDefinedSpan(row,
						col);
	...
	private void paintCell(int row, int column, Graphics g, Rectangle area) {
		...
		area.setBounds(area.x + horizontalMargin / 2, area.y + verticalMargin / 2, area.width
				- horizontalMargin, area.height - verticalMargin);
		...
		if (table.isEditing() && table.getEditingRow() == row && table.getEditingColumn() == column) {
			...
			rendererPane.paintComponent(g, component, table, area.x, area.y, area.width,
					area.height, true);
		}
	}
}
